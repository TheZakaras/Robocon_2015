...
int piston_solfor_racquet = 31; //racquet Swing Piston Solenoids
int piston_solbac_racquet = 33  ;
/*******************************************************************/
...
//m1(Serial1,2);m2(Serial1,1);m3(Serial2,2),m4(Serial2,1)
  m1 = (V_x + V_y + 8*V_t)*12.7*2;  //Subtract |V_t| from absolute value of linear velocity
  m2 = (V_x - V_y - 8*V_t)*12.7*2;
  m4 = (-V_x - V_y + 8*V_t)*12.7*2; //Add |V_t| to absolute value of linear velocity
  m3 = (-V_x + V_y - 8*V_t)*12.7*2; 
...
//    ST2.motor(2, 0);
  ...
  *****************************************************************/
  ...
  {
    Serial.print("Flag High, press R1");
  }
...
void servemotor(int servecount)
{
  ...
  AnThresh = 0;
  for(int i = 0; i<5; i++)
    AnThresh = AnThresh + analogRead(A3);
  AnThresh = AnThresh/5 - 100;
  //AnThresh=500;
  digitalWrite(servemotor1b,HIGH);  //Motor 1: move outward
  while(analogRead(A3)>AnThresh)
  {
    Serial.println(analogRead(A3));
    Serial.print("Sensing IR Threshold");
    Serial.print(AnThresh);
    Serial.println();
  }  //sensing shuttle
   Serial.println(analogRead(A3));
